...
export async function getRoadmap(slug: string) {
  ...
  if (!roadmap) notFound();
  const [{ data: levels, error: levelsError }, { data: enrollment, error: enrollmentError }, { data: progress, error: progressError }] = await Promise.all([
    ...
    supabase.from("user_level_progress").select("level_id").eq("user_id", user.id),
  ]);
  if (levelsError || enrollmentError || progressError) throw new Error("Unable to load your roadmap progress.");
