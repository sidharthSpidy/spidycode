...
export async function updateSession(request: NextRequest) {
  ...
  const protectedPaths = ["/dashboard", "/roadmaps", "/projects", "/leaderboard", "/profile", "/settings"];
  if (!user && protectedPaths.some((protectedPath) => path.startsWith(protectedPath))) { const redirectUrl = request.nextUrl.clone(); redirectUrl.pathname = "/login"; redirectUrl.searchParams.set("next", path); return NextResponse.redirect(redirectUrl); }
